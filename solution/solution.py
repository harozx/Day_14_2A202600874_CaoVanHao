...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        for chunk in contexts:
            ...
            coverage = len(chunk_tokens & expected_tokens) / len(expected_tokens) if len(expected_tokens) > 0 else 1.0
            is_relevant.append(coverage >= relevance_threshold)

        num_relevant = sum(1 for r in is_relevant if r)
        if num_relevant == 0:
            return 0.0
...
class LLMJudge:
        ...
        try:
            ...
            if isinstance(scores, dict):
                if "scores" in scores and isinstance(scores["scores"], dict):
                    ...
                    reasoning = scores.get("reasoning", raw_response)
                else:
                    actual_scores = {}
            ...
            actual_scores = {criterion: 0.5 for criterion in rubric.keys()}
            reasoning = raw_response
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        overall_avg = sum(all_scores_flat) / len(all_scores_flat) if all_scores_flat else 0.5
        ...
        for qa in qa_pairs:
            ...
            if qa.retrieved_contexts:
                eval_result.context_precision = evaluator.evaluate_context_precision(
                    qa.retrieved_contexts, qa.expected_answer
                )
    ...
    def run_regression(self, new_results: list[EvalResult], baseline_results: list[EvalResult]) -> dict:
        ...
        def get_avg(results):
            if not results:
                return 0.0, 0.0, 0.0
            n = len(results)
            f = sum(r.faithfulness for r in results) / n
            rel = sum(r.relevance for r in results) / n
            c = sum(r.completeness for r in results) / n
            return f, rel, c

        new_f, new_r, new_c = get_avg(new_results)
        base_f, base_r, base_c = get_avg(baseline_results)
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        min_keys = [s[0] for s in scores if s[1] == min_val]
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for idx, f in enumerate(failures):
            ...
            if suggestions:
                suggested_fix = suggestions[idx % len(suggestions)]
            else:
                suggested_fix = "Review pipeline configuration"
                
            rows.append(f"| {f_id} | {f_type} | {root_cause} | {suggested_fix} | Open |")
    ...
    def generate_improvement_suggestions(
        ...
        if counts.get("hallucination", 0) > 0:
            ...
            suggestions.append("Refine retriever weights or search query expansion to fetch better context")
        ...
        if counts.get("irrelevant", 0) > 0 or counts.get("off_topic", 0) > 0:
            suggestions.append("Improve prompt clarity and relevance constraints by adding explicit guidelines")
            suggestions.append("Refine intent detection routing to match queries with the correct agent tools")
